...
exports.createClient = function(port, host, user, pass, maxListeners, maxSockets) {
  if (isNaN(port)) {
    host = port;
    port = 5984;
  }
  ...
  var
    ...
    _addClientListenerArgs;
    
  if (typeof maxListeners !== "undefined"){
    // [todo] httpClient.setMaxListeners(maxListeners);
  }
  ...
  couchClient._addClientListener = function() {
    _addClientListenerArgs = arguments;
	  // [todo] httpClient.addListener.apply(httpClient, arguments);
  };
  ...
  couchClient._queueRequest = function(options, cb) {
    ...
    if (_addClientListenerArgs) {
      request.on.apply(request, _addClientListenerArgs);
    }
